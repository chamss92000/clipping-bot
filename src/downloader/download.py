...
def _ydl_opts(dest_dir: Path, outtmpl: str, window: tuple[float, float] | None) -> dict:
    ...
    if cookies and Path(cookies).exists():
        # Session authentifiée : on garde les clients PAR DÉFAUT. Forcer un
        # client alternatif (tv/mweb) casse l'auth ("The page needs to be
        # reloaded").
        opts["cookiefile"] = cookies
        log.debug("yt-dlp: cookies YouTube utilisés (%s)", cookies)
    if settings.YTDLP_PLAYER_CLIENTS:
        # Optionnel et désactivé par défaut : forcer un client casse plus qu'il
        # ne répare (cf. commentaire dans settings).
        opts["extractor_args"] = {"youtube": {"player_client": settings.YTDLP_PLAYER_CLIENTS}}
